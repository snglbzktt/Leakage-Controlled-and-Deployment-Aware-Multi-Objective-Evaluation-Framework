...
ENGINE_VERSION = "phase5_ptq_engine_v3_2"


class PTQModelWrapper(nn.Module):
    def __init__(
        self,
...
def get_prepare():
    ...
    raise RuntimeError(
        "PTQ prepare API is unavailable."
    )
...
def get_convert():
    ...
    raise RuntimeError(
        "PTQ convert API is unavailable."
    )
...
def get_default_qconfig(
    ...
    raise RuntimeError(
        "Default PTQ qconfig API is unavailable."
    )
...
def prepare_ptq_model(
    ...
    wrapped = PTQModelWrapper(
        copy.deepcopy(source_model)
    )

    wrapped.eval()
    wrapped.qconfig = get_default_qconfig(
        backend
    )

    prepared = get_prepare()(
        wrapped,
        inplace=False,
    )
