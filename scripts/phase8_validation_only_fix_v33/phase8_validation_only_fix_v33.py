...
def validate_no_test_fields_in_selection_artifacts(
    ...
    protocol_text = json.dumps(
        {
            "eligibility_profiles": protocol[
                "eligibility_profiles"
            ],
            "Pareto_analysis": protocol["Pareto_analysis"],
            "primary_selection_rule": protocol[
    ...
    if "test_" in protocol_text:
        offenders.append("protocol:test_ token")

    if offenders:
        raise RuntimeError(
            "Forbidden test field entered selection artifacts: "
            + ", ".join(offenders)
        )
