...
def resolve_architecture(
    ...
    if len(valid) == 0:
        ...
        raise RuntimeError(
            "No valid "
            f"{architecture} implementation was found. "
            f"Module nn.Module classes: "
            f"{module_classes}. "
            f"Attempts: {attempts}"
        )

    # A named class and a generic factory can legitimately construct the
    # same architecture. Select the unique highest-scoring symbol rather
    # than treating equivalent aliases as an architecture ambiguity.
    valid.sort(
        key=lambda item: (
            -item[0],
            item[1],
    ...
    if len(top_candidates) != 1:
        raise RuntimeError(
            "Architecture resolution has "
            f"{len(top_candidates)} equally ranked "
            f"top candidates for {architecture}: "
            f"{[item[1] for item in top_candidates]}. "
            f"Attempts: {attempts}"
        )
    ...
    resolution = {
        ...
        "selection_policy": (
            "unique highest architecture-name score; "
            "lower-ranked equivalent factories are aliases"
        ),
        "valid_symbols": [
            item[1]
            for item in valid
