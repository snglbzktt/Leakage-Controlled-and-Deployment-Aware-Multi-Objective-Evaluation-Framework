...
def family_code_from_class_name(class_name: str) -> int:
    """Map canonical N-BaIoT subtype labels to the locked family_3 target."""
    normalized = str(class_name).strip().lower()

    if normalized == "benign":
        return FAMILY_CODES["benign"]

    if normalized.startswith("gafgyt_"):
        return FAMILY_CODES["gafgyt"]

    if normalized.startswith("mirai_"):
        return FAMILY_CODES["mirai"]
...
with sqlite3.connect(
    ...
    for batch_number, batch in enumerate(
        ...
        for (
            ...
            family_code = family_code_from_class_name(
                class_name
            )
