...
# Keep CPU use predictable on the 16 GB host.
...
def main() -> None:
    ...
    if SUMMARY_JSON.exists():
        raise FileExistsError(
            "HGB summary already exists; "
            f"refusing to overwrite: {SUMMARY_JSON}"
        )
    ...
    random_forest_verification = read_json(
        RANDOM_FOREST_VERIFICATION
    )
    ...
    checks = {
        ...
        "random_forest_verification_passed": (
            random_forest_verification.get("status")
            == "passed"
    ...
    if failed_checks:
        raise RuntimeError(
            "HGB preflight failed: "
            + ", ".join(failed_checks)
        )

    if output_directory.exists():
        raise FileExistsError(
            "HGB output directory already exists; "
    ...
    print("=" * 92)
    print("PHASE 4 CLASSICAL BASELINE: HIST GRADIENT BOOSTING B0")
    print("=" * 92)
    ...
    print()
    print("Training locked HGB model...")
    ...
    np.savez_compressed(
        ...
        n_trees_per_iteration=np.asarray(
            [model.n_trees_per_iteration_],
            dtype=np.int64,
        ),
        do_early_stopping=np.asarray(
            [bool(model.do_early_stopping_)],
            dtype=np.bool_,
        ),
    )
    ...
    run_manifest = {
        ...
        "final_cache_verification": str(
            FINAL_CACHE_VERIFICATION
        ),
        "final_cache_verification_sha256": (
            sha256_file(
                FINAL_CACHE_VERIFICATION
            )
        ),
        "logistic_verification": str(
            LOGISTIC_VERIFICATION
        ),
        "logistic_verification_sha256": (
            sha256_file(
                LOGISTIC_VERIFICATION
            )
        ),
        ...
        "random_forest_verification": str(
            RANDOM_FOREST_VERIFICATION
        ),
        "random_forest_verification_sha256": (
            sha256_file(
                RANDOM_FOREST_VERIFICATION
            )
        ),
    ...
    print()
    print("=" * 92)
    print("HIST GRADIENT BOOSTING B0 RESULT")
    print("=" * 92)
    print(
        "Run ID                         : "
        f"{run_id}"
    ...
    print(
        "HIST GRADIENT BOOSTING B0 COMPLETED"
    )
