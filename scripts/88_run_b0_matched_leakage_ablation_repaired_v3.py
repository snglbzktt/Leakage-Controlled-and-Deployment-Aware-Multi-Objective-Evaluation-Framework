...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="N-BaIoT cihaz klasÃƒÆ’Ã‚Â¶rlerini iÃƒÆ’Ã‚Â§eren kÃƒÆ’Ã‚Â¶k dizin.",
    )
    ...
    parser.add_argument(
        "--restart-incomplete",
        action="store_true",
        help="YalnÃƒâ€Ã‚Â±z tamamlanmamÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ VS2 run klasÃƒÆ’Ã‚Â¶rlerini silip yeniden baÃƒâ€¦Ã…Â¸latÃƒâ€Ã‚Â±r.",
    )
...
def json_default(value: object) -> object:
    ...
    if isinstance(value, Path):
        return str(value)
    raise TypeError(f"{type(value).__name__} JSON ile uyumlu deÃƒâ€Ã…Â¸il.")
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(document, dict):
        raise TypeError(f"JSON kÃƒÆ’Ã‚Â¶kÃƒÆ’Ã‚Â¼ sÃƒÆ’Ã‚Â¶zlÃƒÆ’Ã‚Â¼k deÃƒâ€Ã…Â¸il: {path}")
    return document
...
def database_metadata(database: Path) -> dict[str, str]:
    ...
    for key, value in expected.items():
        if metadata.get(key) != value:
            raise RuntimeError(f"Split veritabanÃƒâ€Ã‚Â± metadata uyuÃƒâ€¦Ã…Â¸mazlÃƒâ€Ã‚Â±Ãƒâ€Ã…Â¸Ãƒâ€Ã‚Â±: {key}={metadata.get(key)!r}")
    return metadata
...
def source_database_path(database: Path) -> Path:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Kaynak parmak izi veritabanÃƒâ€Ã‚Â± bulunamadÃƒâ€Ã‚Â±: {path}")
    return path.resolve()
...
def discover_files(source_database: Path, csv_root: Path) -> list[tuple[int, Path, str, int]]:
    with sqlite3.connect(source_database) as connection:
        ...
        if path_column is None or not required.issubset(columns):
            raise RuntimeError(f"Desteklenmeyen source.files Ãƒâ€¦Ã…Â¸emasÃƒâ€Ã‚Â±: {columns}")
    ...
    for file_id, raw_path, label, expected_rows in rows:
        ...
        if resolved is None:
            ...
            raise FileNotFoundError(
                f"CSV bulunamadÃƒâ€Ã‚Â± (file_id={file_id}): {raw_path}\n"
                f"Denenen yollar:\n  - {attempted}\n"
                "Veri farklÃƒâ€Ã‚Â± klasÃƒÆ’Ã‚Â¶rdeyse --csv-root ile N-BaIoT kÃƒÆ’Ã‚Â¶kÃƒÆ’Ã‚Â¼nÃƒÆ’Ã‚Â¼ belirtin."
            )
...
def split_counts(database: Path, arm: str) -> dict[str, int]:
    ...
    if set(by_code) != {0, 1, 2}:
        raise RuntimeError(f"{arm} split kodlarÃƒâ€Ã‚Â± geÃƒÆ’Ã‚Â§ersiz: {by_code}")
    result = {name: by_code[code] for name, code in SPLIT_CODES.items()}
    if sum(result.values()) != EXPECTED_RAW_ROW_COUNT:
        raise RuntimeError(f"{arm} toplam kayÃƒâ€Ã‚Â±t sayÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â± geÃƒÆ’Ã‚Â§ersiz: {sum(result.values()):,}")
    return result
...
def split_class_counts(database: Path, arm: str) -> dict[str, list[int]]:
    ...
    for split, values in result.items():
        if any(value <= 0 for value in values):
            raise RuntimeError(f"{arm}/{split}: eksik family_3 sÃƒâ€Ã‚Â±nÃƒâ€Ã‚Â±fÃƒâ€Ã‚Â±: {values}")
    return result
...
def validate_database(database: Path) -> dict[str, Any]:
    ...
    with sqlite3.connect(database) as connection:
        ...
        if quick_check.lower() != "ok":
            raise RuntimeError(f"Split veritabanÃƒâ€Ã‚Â± quick_check baÃƒâ€¦Ã…Â¸arÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â±z: {quick_check}")
    ...
    if failed:
        raise RuntimeError(f"Stage-37 bilimsel kilitleri baÃƒâ€¦Ã…Â¸arÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â±z: {failed}")
    return {
        "metadata": metadata,
...
def validate_source_files(database: Path, csv_root: Path) -> dict[str, Any]:
    ...
    if observed_source_database_sha256 != expected_source_database_sha256:
        raise RuntimeError(
            "Kaynak denetim veritabanÃƒâ€Ã‚Â± SHA-256 deÃƒâ€Ã…Â¸eri stage 37 sonrasÃƒâ€Ã‚Â±nda deÃƒâ€Ã…Â¸iÃƒâ€¦Ã…Â¸miÃƒâ€¦Ã…Â¸."
        )
    discovered = {
    ...
    if len(integrity_rows) != expected_file_count or set(discovered) != {
        ...
        raise RuntimeError("Kaynak CSV envanteri stage 37 ile uyuÃƒâ€¦Ã…Â¸muyor.")
    total_bytes = 0
    for position, row in enumerate(integrity_rows, start=1):
        ...
        if current_path.resolve() != Path(str(stored_path)).resolve():
            raise RuntimeError(f"Kaynak CSV yolu deÃƒâ€Ã…Â¸iÃƒâ€¦Ã…Â¸miÃƒâ€¦Ã…Â¸ (file_id={file_id}).")
        if current_label != str(stored_label) or current_rows != int(stored_rows):
            raise RuntimeError(f"Kaynak CSV etiketi/satÃƒâ€Ã‚Â±r sayÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â± deÃƒâ€Ã…Â¸iÃƒâ€¦Ã…Â¸miÃƒâ€¦Ã…Â¸ (file_id={file_id}).")
        stat = current_path.stat()
        if stat.st_size != int(stored_size):
            raise RuntimeError(f"Kaynak CSV boyutu deÃƒâ€Ã…Â¸iÃƒâ€¦Ã…Â¸miÃƒâ€¦Ã…Â¸: {current_path}")
        observed_sha256 = sha256(current_path)
        if observed_sha256 != str(stored_sha256):
            raise RuntimeError(f"Kaynak CSV SHA-256 uyuÃƒâ€¦Ã…Â¸muyor: {current_path}")
        total_bytes += int(stat.st_size)
        print(
            f"[kaynak doÃƒâ€Ã…Â¸rulama] {position}/{len(integrity_rows)}: {current_path.name}",
            flush=True,
        )
    return {
...
def inverse_sqrt_mean1(class_counts: np.ndarray) -> np.ndarray:
    ...
    if counts.shape != (3,) or np.any(counts <= 0):
        raise RuntimeError(f"GeÃƒÆ’Ã‚Â§ersiz eÃƒâ€Ã…Â¸itim sÃƒâ€Ã‚Â±nÃƒâ€Ã‚Â±f sayÃƒâ€Ã‚Â±larÃƒâ€Ã‚Â±: {counts.tolist()}")
...
def float32_fingerprints(values: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if matrix.ndim != 2 or matrix.shape[1] != EXPECTED_FEATURE_COUNT:
        raise RuntimeError(f"Model girdisi boyutu geÃƒÆ’Ã‚Â§ersiz: {matrix.shape}")
    if not np.isfinite(matrix).all():
        raise RuntimeError("Model girdisinde sonlu olmayan deÃƒâ€Ã…Â¸er var.")
...
def audit_scaled_grouped_overlap(
    ...
    try:
        ...
        for split_code, split in enumerate(SPLITS):
            ...
            for start in range(0, len(x), chunk_size):
                ...
                if now - last_progress >= 60.0:
                    print(
                        f"[grouped_float32 gerÃƒÆ’Ã‚Â§ek model-girdisi denetimi] {split}: "
        ...
        quick_check = str(connection.execute("PRAGMA quick_check").fetchone()[0])
        if quick_check.lower() != "ok":
            raise RuntimeError(f"ÃƒÆ’Ã¢â‚¬â€œlÃƒÆ’Ã‚Â§eklenmiÃƒâ€¦Ã…Â¸ girdi denetim DB quick_check baÃƒâ€¦Ã…Â¸arÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â±z: {quick_check}")
    finally:
        connection.close()
    if processed_rows != EXPECTED_RAW_ROW_COUNT:
        raise RuntimeError(
            f"ÃƒÆ’Ã¢â‚¬â€œlÃƒÆ’Ã‚Â§eklenmiÃƒâ€¦Ã…Â¸ girdi denetim satÃƒâ€Ã‚Â±r sayÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â± geÃƒÆ’Ã‚Â§ersiz: "
            f"{processed_rows:,} != {EXPECTED_RAW_ROW_COUNT:,}"
        )
    if set(unique_by_split) != set(SPLITS):
        raise RuntimeError(f"ÃƒÆ’Ã¢â‚¬â€œlÃƒÆ’Ã‚Â§eklenmiÃƒâ€¦Ã…Â¸ girdi split envanteri eksik: {unique_by_split}")
    if overlap_count != 0:
        raise RuntimeError(
            "Grouped kolda StandardScaler ve float32 dÃƒÆ’Ã‚Â¶nÃƒÆ’Ã‚Â¼Ãƒâ€¦Ã…Â¸ÃƒÆ’Ã‚Â¼mÃƒÆ’Ã‚Â¼ sonrasÃƒâ€Ã‚Â±nda splitler arasÃƒâ€Ã‚Â± "
            f"{overlap_count:,} model-girdisi ÃƒÆ’Ã‚Â§akÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸masÃƒâ€Ã‚Â± bulundu; eÃƒâ€Ã…Â¸itim gÃƒÆ’Ã‚Â¼venli biÃƒÆ’Ã‚Â§imde durduruldu."
        )
    return {
...
def ensure_cache_disk_space(database: Path, cache: Path, arms: list[str]) -> None:
    ...
    if free < required:
        raise RuntimeError(
            "VS2 ÃƒÆ’Ã‚Â¶nbelleÃƒâ€Ã…Â¸i iÃƒÆ’Ã‚Â§in boÃƒâ€¦Ã…Â¸ alan yetersiz: "
            f"gereken yaklaÃƒâ€¦Ã…Â¸Ãƒâ€Ã‚Â±k={required / 1024**3:.2f} GiB, "
...
def validate_completed_cache(
    ...
    if mismatches:
        raise RuntimeError(f"Eski/uyumsuz VS2 ÃƒÆ’Ã‚Â¶nbelleÃƒâ€Ã…Â¸i bulundu ({arm}): {mismatches}")
    for split in SPLITS:
        x = np.load(arm_directory / f"X_{split}.npy", mmap_mode="r")
        y = np.load(arm_directory / f"y_{split}.npy", mmap_mode="r")
        if x.shape != (counts[split], EXPECTED_FEATURE_COUNT) or x.dtype != np.float32:
            raise RuntimeError(f"GeÃƒÆ’Ã‚Â§ersiz ÃƒÆ’Ã‚Â¶nbellek X dizisi: {arm}/{split} {x.shape} {x.dtype}")
        if y.shape != (counts[split],) or y.dtype != np.int64:
            raise RuntimeError(f"GeÃƒÆ’Ã‚Â§ersiz ÃƒÆ’Ã‚Â¶nbellek y dizisi: {arm}/{split} {y.shape} {y.dtype}")
        observed_counts = np.bincount(y, minlength=3).astype(np.int64).tolist()
        if observed_counts != expected_class_counts[split]:
            raise RuntimeError(
                f"ÃƒÆ’Ã¢â‚¬â€œnbellek sÃƒâ€Ã‚Â±nÃƒâ€Ã‚Â±f sayÃƒâ€Ã‚Â±larÃƒâ€Ã‚Â± uyuÃƒâ€¦Ã…Â¸muyor: {arm}/{split} "
    ...
    if mean.shape != (EXPECTED_FEATURE_COUNT,) or mean.dtype != np.float64:
        raise RuntimeError(f"GeÃƒÆ’Ã‚Â§ersiz scaler mean: {arm} {mean.shape} {mean.dtype}")
    if scale.shape != (EXPECTED_FEATURE_COUNT,) or scale.dtype != np.float64:
        raise RuntimeError(f"GeÃƒÆ’Ã‚Â§ersiz scaler scale: {arm} {scale.shape} {scale.dtype}")
    if weights.shape != (3,) or weights.dtype != np.float32:
        raise RuntimeError(f"GeÃƒÆ’Ã‚Â§ersiz sÃƒâ€Ã‚Â±nÃƒâ€Ã‚Â±f aÃƒâ€Ã…Â¸Ãƒâ€Ã‚Â±rlÃƒâ€Ã‚Â±Ãƒâ€Ã…Â¸Ãƒâ€Ã‚Â±: {arm} {weights.shape} {weights.dtype}")
    ...
    if not isinstance(split_audit, dict):
        raise RuntimeError(f"ÃƒÆ’Ã¢â‚¬â€œnbellekte ÃƒÆ’Ã‚Â¶lÃƒÆ’Ã‚Â§eklenmiÃƒâ€¦Ã…Â¸ split tam-tarama denetimi yok: {arm}")
    for split in SPLITS:
        ...
        if (
            ...
            raise RuntimeError(f"ÃƒÆ’Ã¢â‚¬â€œlÃƒÆ’Ã‚Â§eklenmiÃƒâ€¦Ã…Â¸ split denetimi geÃƒÆ’Ã‚Â§ersiz: {arm}/{split}: {item}")
    overlap_audit = manifest.get("scaled_model_input_overlap_audit")
    if arm == "grouped_float32":
        if not isinstance(overlap_audit, dict):
            raise RuntimeError("Grouped ÃƒÆ’Ã‚Â¶nbellekte ÃƒÆ’Ã‚Â¶lÃƒÆ’Ã‚Â§eklenmiÃƒâ€¦Ã…Â¸ model-girdisi denetimi yok.")
        if (
            ...
            raise RuntimeError(f"Grouped ÃƒÆ’Ã‚Â¶lÃƒÆ’Ã‚Â§eklenmiÃƒâ€¦Ã…Â¸ model-girdisi denetimi geÃƒÆ’Ã‚Â§ersiz: {overlap_audit}")
        audit_database = Path(str(overlap_audit.get("audit_database", "")))
        if not audit_database.exists():
            raise RuntimeError(f"Grouped ÃƒÆ’Ã‚Â¶lÃƒÆ’Ã‚Â§eklenmiÃƒâ€¦Ã…Â¸ girdi denetim DB bulunamadÃƒâ€Ã‚Â±: {audit_database}")
        if sha256(audit_database) != overlap_audit.get("audit_database_sha256"):
            raise RuntimeError("Grouped ÃƒÆ’Ã‚Â¶lÃƒÆ’Ã‚Â§eklenmiÃƒâ€¦Ã…Â¸ girdi denetim DB SHA-256 uyuÃƒâ€¦Ã…Â¸muyor.")
...
def materialize_arm(
    ...
    if manifest_file.exists():
        ...
        if manifest.get("status") == "completed":
            ...
            print(f"[{arm}] TamamlanmÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ VS2 ÃƒÆ’Ã‚Â¶nbelleÃƒâ€Ã…Â¸i kullanÃƒâ€Ã‚Â±lÃƒâ€Ã‚Â±yor.", flush=True)
            return manifest
    ...
    for file_number, (file_id, csv_path, raw_label, expected_rows) in enumerate(files, start=1):
        ...
        for frame in pd.read_csv(csv_path, chunksize=chunk_size):
            ...
            if values.shape[1] != EXPECTED_FEATURE_COUNT or not np.isfinite(values).all():
                raise RuntimeError(f"GeÃƒÆ’Ã‚Â§ersiz CSV ÃƒÆ’Ã‚Â¶zellikleri: {csv_path}")
            end = base + len(values)
            for split in SPLITS:
            ...
            if now - last_file_progress >= 60.0:
                print(
                    f"[{arm}] Dosya {file_number}/{len(files)} iÃƒâ€¦Ã…Â¸leniyor: {csv_path.name} "
                    f"({base:,}/{expected_rows:,} satÃƒâ€Ã‚Â±r)",
                    flush=True,
                )
                last_file_progress = now
        if base != expected_rows:
            raise RuntimeError(
                f"CSV satÃƒâ€Ã‚Â±r sayÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â± uyuÃƒâ€¦Ã…Â¸muyor ({csv_path.name}): {base:,} != {expected_rows:,}"
            )
        print(f"[{arm}] Dosya {file_number}/{len(files)} hazÃƒâ€Ã‚Â±rlandÃƒâ€Ã‚Â±: {csv_path.name}", flush=True)
    ...
    for split in SPLITS:
        ...
        if observed_counts != expected_class_counts[split]:
            raise RuntimeError(
                f"{arm}/{split}: materyalize sÃƒâ€Ã‚Â±nÃƒâ€Ã‚Â±f sayÃƒâ€Ã‚Â±larÃƒâ€Ã‚Â± uyuÃƒâ€¦Ã…Â¸muyor: "
                f"{observed_counts} != {expected_class_counts[split]}"
            )
    ...
    if mean.shape != (EXPECTED_FEATURE_COUNT,) or standard_deviation.shape != (
        ...
        raise RuntimeError(f"{arm}: StandardScaler parametre boyutu geÃƒÆ’Ã‚Â§ersiz.")
    if not np.isfinite(mean).all() or not np.isfinite(standard_deviation).all():
        raise RuntimeError(f"{arm}: StandardScaler sonlu olmayan parametre ÃƒÆ’Ã‚Â¼retti.")
    if np.any(standard_deviation <= 0):
        raise RuntimeError(f"{arm}: StandardScaler pozitif olmayan ÃƒÆ’Ã‚Â¶lÃƒÆ’Ã‚Â§ek ÃƒÆ’Ã‚Â¼retti.")
    ...
    if not scaler_audit_passed:
        raise RuntimeError(
            f"{arm}: StandardScaler tam tarama denetimi baÃƒâ€¦Ã…Â¸arÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â±z; "
    ...
    for split in SPLITS:
        ...
        if not split_finite or processed != counts[split]:
            raise RuntimeError(
                f"{arm}/{split}: ÃƒÆ’Ã‚Â¶lÃƒÆ’Ã‚Â§eklenmiÃƒâ€¦Ã…Â¸ model-girdisi sonluluk/satÃƒâ€Ã‚Â±r denetimi baÃƒâ€¦Ã…Â¸arÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â±z."
            )
    ...
    if arm == "grouped_float32":
        print(
            "[grouped_float32] StandardScaler sonrasÃƒâ€Ã‚Â± gerÃƒÆ’Ã‚Â§ek float32 model-girdisi "
            "ÃƒÆ’Ã‚Â¶rtÃƒÆ’Ã‚Â¼Ãƒâ€¦Ã…Â¸mesi denetleniyor...",
            flush=True,
        )
...
def protocol_settings(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if tuple(int(seed) for seed in protocol.get("training_seeds", ())) != SEEDS:
        raise RuntimeError(f"B0 protokol seed kÃƒÆ’Ã‚Â¼mesi uyuÃƒâ€¦Ã…Â¸muyor: {protocol.get('training_seeds')!r}")
    if str(protocol.get("task")) != "family_3":
        raise RuntimeError(f"B0 gÃƒÆ’Ã‚Â¶rev adÃƒâ€Ã‚Â± uyuÃƒâ€¦Ã…Â¸muyor: {protocol.get('task')!r}")
    ...
    for key, value in expected.items():
        if str(common.get(key)) != value:
            raise RuntimeError(f"B0 protokolÃƒÆ’Ã‚Â¼ uyuÃƒâ€¦Ã…Â¸muyor: {key}={common.get(key)!r}")
    settings = {
        "optimizer": "AdamW",
    ...
    for key, expected_value in locked.items():
        if settings[key] != expected_value:
            raise RuntimeError(
                f"Kilitli B0 ayarÃƒâ€Ã‚Â± deÃƒâ€Ã…Â¸iÃƒâ€¦Ã…Â¸miÃƒâ€¦Ã…Â¸: {key}={settings[key]!r}, beklenen={expected_value!r}"
            )
...
def train_one(
    ...
    if result_file.exists():
        ...
        if mismatches:
            raise RuntimeError(f"TamamlanmÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ VS2 sonucu uyumsuz: {result_file}: {mismatches}")
        print(f"[{arm}/seed{seed}] TamamlanmÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ VS2 sonucu kullanÃƒâ€Ã‚Â±lÃƒâ€Ã‚Â±yor.", flush=True)
        return result
    if run_directory.exists() and any(run_directory.iterdir()):
        if not restart_incomplete:
            raise RuntimeError(
                f"YarÃƒâ€Ã‚Â±m kalmÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸ run bulundu: {run_directory}. "
                "Ãƒâ€Ã‚Â°nceledikten sonra yalnÃƒâ€Ã‚Â±z bu VS2 run iÃƒÆ’Ã‚Â§in --restart-incomplete kullanÃƒâ€Ã‚Â±n."
            )
    ...
    if parameter_count != EXPECTED_PARAMETER_COUNT:
        raise RuntimeError(
            f"B0 tinyml_mlp parametre sayÃƒâ€Ã‚Â±sÃƒâ€Ã‚Â± deÃƒâ€Ã…Â¸iÃƒâ€¦Ã…Â¸miÃƒâ€¦Ã…Â¸: {parameter_count:,} != {EXPECTED_PARAMETER_COUNT:,}"
        )
...
def main() -> None:
    ...
    if args.chunk_size <= 0:
        raise ValueError("--chunk-size pozitif olmalÃƒâ€Ã‚Â±dÃƒâ€Ã‚Â±r.")
    if tuple(args.seeds) != SEEDS:
        raise ValueError(f"Seed kÃƒÆ’Ã‚Â¼mesi kilitli olmalÃƒâ€Ã‚Â±dÃƒâ€Ã‚Â±r: {SEEDS}")
    if tuple(args.arms) != ARMS:
        print("UYARI: YalnÃƒâ€Ã‚Â±z seÃƒÆ’Ã‚Â§ili kollar ÃƒÆ’Ã‚Â§alÃƒâ€Ã‚Â±Ãƒâ€¦Ã…Â¸tÃƒâ€Ã‚Â±rÃƒâ€Ã‚Â±lÃƒâ€Ã‚Â±yor; nihai eÃƒâ€¦Ã…Â¸leÃƒâ€¦Ã…Â¸tirilmiÃƒâ€¦Ã…Â¸ rapor oluÃƒâ€¦Ã…Â¸mayabilir.")
    ...
    print("Stage-37 bilimsel kilitleri doÃƒâ€Ã…Â¸rulandÃƒâ€Ã‚Â±; kaynak CSV ÃƒÆ’Ã‚Â¶zetleri sÃƒâ€Ã‚Â±nanÃƒâ€Ã‚Â±yor...", flush=True)
    database_validation["source_integrity"] = validate_source_files(database, csv_root)
    print("Kaynak CSV bÃƒÆ’Ã‚Â¼tÃƒÆ’Ã‚Â¼nlÃƒÆ’Ã‚Â¼Ãƒâ€Ã…Â¸ÃƒÆ’Ã‚Â¼ doÃƒâ€Ã…Â¸rulandÃƒâ€Ã‚Â±; veritabanÃƒâ€Ã‚Â± ÃƒÆ’Ã‚Â¶zeti alÃƒâ€Ã‚Â±nÃƒâ€Ã‚Â±yor...", flush=True)
    ...
    if args.prepare_only:
        print("VS2 ÃƒÆ’Ã‚Â¶nbellek hazÃƒâ€Ã‚Â±rlama tamamlandÃƒâ€Ã‚Â±; --prepare-only nedeniyle eÃƒâ€Ã…Â¸itim baÃƒâ€¦Ã…Â¸latÃƒâ€Ã‚Â±lmadÃƒâ€Ã‚Â±.")
        return
    ...
    for arm in args.arms:
        for seed in args.seeds:
            ...
            print(f"[{arm}/seed{seed}] B0-eÃƒâ€¦Ã…Â¸lenmiÃƒâ€¦Ã…Â¸ eÃƒâ€Ã…Â¸itim baÃƒâ€¦Ã…Â¸lÃƒâ€Ã‚Â±yor...", flush=True)
            result = train_one(
                arm_directory=cache / arm,
    ...
    if set(args.arms) == set(ARMS) and set(args.seeds) == set(SEEDS):
        ...
        print("=" * 78)
        print("38. aÃƒâ€¦Ã…Â¸ama tamamlandÃƒâ€Ã‚Â±: B0-eÃƒâ€¦Ã…Â¸lenmiÃƒâ€¦Ã…Â¸ float32 sÃƒâ€Ã‚Â±zÃƒâ€Ã‚Â±ntÃƒâ€Ã‚Â± ablasyonu")
        print(f"Naif Macro-F1 ortalamasÃƒâ€Ã‚Â± : {macro['naive_row'].mean():.9f}")
        print(f"Gruplu Macro-F1 ortalamasÃƒâ€Ã‚Â±: {macro['grouped_float32'].mean():.9f}")
        print(f"Naif - gruplu farkÃƒâ€Ã‚Â±       : {differences.mean():.9f}")
        print("Eski 150 ana koÃƒâ€¦Ã…Â¸u ve VS1 sonuÃƒÆ’Ã‚Â§larÃƒâ€Ã‚Â± deÃƒâ€Ã…Â¸iÃƒâ€¦Ã…Â¸tirilmedi.")
        print("=" * 78)
