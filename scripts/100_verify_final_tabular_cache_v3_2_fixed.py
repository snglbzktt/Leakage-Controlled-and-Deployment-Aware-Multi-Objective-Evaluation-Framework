...
if not candidate_model_columns:
    raise RuntimeError(
...
for candidate_column in candidate_model_columns:
    counts: dict[str, int] = {}
...
matching_model_columns = [
    name
    for name, counts
    in candidate_model_counts.items()
    if counts
    == EXPECTED_MODEL_RUN_COUNTS
]
...
if not matching_model_columns:
    raise RuntimeError(
        "No run-matrix column matches the locked "
        "model/run counts. Candidates: "
        f"{candidate_model_counts}"
    )

model_column = (
    matching_model_columns[0]
)
