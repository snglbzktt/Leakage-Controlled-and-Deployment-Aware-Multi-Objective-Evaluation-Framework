...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="N-BaIoT cihaz klasÃ¶rlerini iÃ§eren kÃ¶k dizin.",
    )
    ...
    parser.add_argument(
        "--restart-incomplete",
        action="store_true",
        help="YalnÄ±z tamamlanmamÄ±ÅŸ VS2 run klasÃ¶rlerini silip yeniden baÅŸlatÄ±r.",
    )
...
def json_default(value: object) -> object:
    ...
    if isinstance(value, Path):
        return str(value)
    raise TypeError(f"{type(value).__name__} JSON ile uyumlu deÄŸil.")
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(document, dict):
        raise TypeError(f"JSON kÃ¶kÃ¼ sÃ¶zlÃ¼k deÄŸil: {path}")
    return document
...
def database_metadata(database: Path) -> dict[str, str]:
    ...
    for key, value in expected.items():
        if metadata.get(key) != value:
            raise RuntimeError(f"Split veritabanÄ± metadata uyuÅŸmazlÄ±ÄŸÄ±: {key}={metadata.get(key)!r}")
    return metadata
...
def source_database_path(database: Path) -> Path:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Kaynak parmak izi veritabanÄ± bulunamadÄ±: {path}")
    return path.resolve()
...
def discover_files(source_database: Path, csv_root: Path) -> list[tuple[int, Path, str, int]]:
    with sqlite3.connect(source_database) as connection:
        ...
        if path_column is None or not required.issubset(columns):
            raise RuntimeError(f"Desteklenmeyen source.files ÅŸemasÄ±: {columns}")
    ...
    for file_id, raw_path, label, expected_rows in rows:
        ...
        if resolved is None:
            ...
            raise FileNotFoundError(
                f"CSV bulunamadÄ± (file_id={file_id}): {raw_path}\n"
                f"Denenen yollar:\n  - {attempted}\n"
                "Veri farklÄ± klasÃ¶rdeyse --csv-root ile N-BaIoT kÃ¶kÃ¼nÃ¼ belirtin."
            )
...
def split_counts(database: Path, arm: str) -> dict[str, int]:
    ...
    if set(by_code) != {0, 1, 2}:
        raise RuntimeError(f"{arm} split kodlarÄ± geÃ§ersiz: {by_code}")
    result = {name: by_code[code] for name, code in SPLIT_CODES.items()}
    if sum(result.values()) != EXPECTED_RAW_ROW_COUNT:
        raise RuntimeError(f"{arm} toplam kayÄ±t sayÄ±sÄ± geÃ§ersiz: {sum(result.values()):,}")
    return result
...
def split_class_counts(database: Path, arm: str) -> dict[str, list[int]]:
    ...
    for split, values in result.items():
        if any(value <= 0 for value in values):
            raise RuntimeError(f"{arm}/{split}: eksik family_3 sÄ±nÄ±fÄ±: {values}")
    return result
...
def validate_database(database: Path) -> dict[str, Any]:
    ...
    with sqlite3.connect(database) as connection:
        ...
        if quick_check.lower() != "ok":
            raise RuntimeError(f"Split veritabanÄ± quick_check baÅŸarÄ±sÄ±z: {quick_check}")
    ...
    if failed:
        raise RuntimeError(f"Stage-37 bilimsel kilitleri baÅŸarÄ±sÄ±z: {failed}")
    return {
        "metadata": metadata,
...
def validate_source_files(database: Path, csv_root: Path) -> dict[str, Any]:
    ...
    if observed_source_database_sha256 != expected_source_database_sha256:
        raise RuntimeError(
            "Kaynak denetim veritabanÄ± SHA-256 deÄŸeri stage 37 sonrasÄ±nda deÄŸiÅŸmiÅŸ."
        )
    discovered = {
    ...
    if len(integrity_rows) != expected_file_count or set(discovered) != {
        ...
        raise RuntimeError("Kaynak CSV envanteri stage 37 ile uyuÅŸmuyor.")
    total_bytes = 0
    for position, row in enumerate(integrity_rows, start=1):
        ...
        if current_path.resolve() != Path(str(stored_path)).resolve():
            raise RuntimeError(f"Kaynak CSV yolu deÄŸiÅŸmiÅŸ (file_id={file_id}).")
        if current_label != str(stored_label) or current_rows != int(stored_rows):
            raise RuntimeError(f"Kaynak CSV etiketi/satÄ±r sayÄ±sÄ± deÄŸiÅŸmiÅŸ (file_id={file_id}).")
        stat = current_path.stat()
        if stat.st_size != int(stored_size):
            raise RuntimeError(f"Kaynak CSV boyutu deÄŸiÅŸmiÅŸ: {current_path}")
        observed_sha256 = sha256(current_path)
        if observed_sha256 != str(stored_sha256):
            raise RuntimeError(f"Kaynak CSV SHA-256 uyuÅŸmuyor: {current_path}")
        total_bytes += int(stat.st_size)
        print(
            f"[kaynak doÄŸrulama] {position}/{len(integrity_rows)}: {current_path.name}",
            flush=True,
        )
    return {
...
def inverse_sqrt_mean1(class_counts: np.ndarray) -> np.ndarray:
    ...
    if counts.shape != (3,) or np.any(counts <= 0):
        raise RuntimeError(f"GeÃ§ersiz eÄŸitim sÄ±nÄ±f sayÄ±larÄ±: {counts.tolist()}")
...
def float32_fingerprints(values: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if matrix.ndim != 2 or matrix.shape[1] != EXPECTED_FEATURE_COUNT:
        raise RuntimeError(f"Model girdisi boyutu geÃ§ersiz: {matrix.shape}")
    if not np.isfinite(matrix).all():
        raise RuntimeError("Model girdisinde sonlu olmayan deÄŸer var.")
...
def audit_scaled_grouped_overlap(
    ...
    try:
        ...
        for split_code, split in enumerate(SPLITS):
            ...
            for start in range(0, len(x), chunk_size):
                ...
                if now - last_progress >= 60.0:
                    print(
                        f"[grouped_float32 gerÃ§ek model-girdisi denetimi] {split}: "
        ...
        quick_check = str(connection.execute("PRAGMA quick_check").fetchone()[0])
        if quick_check.lower() != "ok":
            raise RuntimeError(f"Ã–lÃ§eklenmiÅŸ girdi denetim DB quick_check baÅŸarÄ±sÄ±z: {quick_check}")
    finally:
        connection.close()
    if processed_rows != EXPECTED_RAW_ROW_COUNT:
        raise RuntimeError(
            f"Ã–lÃ§eklenmiÅŸ girdi denetim satÄ±r sayÄ±sÄ± geÃ§ersiz: "
            f"{processed_rows:,} != {EXPECTED_RAW_ROW_COUNT:,}"
        )
    if set(unique_by_split) != set(SPLITS):
        raise RuntimeError(f"Ã–lÃ§eklenmiÅŸ girdi split envanteri eksik: {unique_by_split}")
    if overlap_count != 0:
        raise RuntimeError(
            "Grouped kolda StandardScaler ve float32 dÃ¶nÃ¼ÅŸÃ¼mÃ¼ sonrasÄ±nda splitler arasÄ± "
            f"{overlap_count:,} model-girdisi Ã§akÄ±ÅŸmasÄ± bulundu; eÄŸitim gÃ¼venli biÃ§imde durduruldu."
        )
    return {
...
def ensure_cache_disk_space(database: Path, cache: Path, arms: list[str]) -> None:
    ...
    if free < required:
        raise RuntimeError(
            "VS2 Ã¶nbelleÄŸi iÃ§in boÅŸ alan yetersiz: "
            f"gereken yaklaÅŸÄ±k={required / 1024**3:.2f} GiB, "
...
def validate_completed_cache(
    ...
    if mismatches:
        raise RuntimeError(f"Eski/uyumsuz VS2 Ã¶nbelleÄŸi bulundu ({arm}): {mismatches}")
    for split in SPLITS:
        x = np.load(arm_directory / f"X_{split}.npy", mmap_mode="r")
        y = np.load(arm_directory / f"y_{split}.npy", mmap_mode="r")
        if x.shape != (counts[split], EXPECTED_FEATURE_COUNT) or x.dtype != np.float32:
            raise RuntimeError(f"GeÃ§ersiz Ã¶nbellek X dizisi: {arm}/{split} {x.shape} {x.dtype}")
        if y.shape != (counts[split],) or y.dtype != np.int64:
            raise RuntimeError(f"GeÃ§ersiz Ã¶nbellek y dizisi: {arm}/{split} {y.shape} {y.dtype}")
        observed_counts = np.bincount(y, minlength=3).astype(np.int64).tolist()
        if observed_counts != expected_class_counts[split]:
            raise RuntimeError(
                f"Ã–nbellek sÄ±nÄ±f sayÄ±larÄ± uyuÅŸmuyor: {arm}/{split} "
    ...
    if mean.shape != (EXPECTED_FEATURE_COUNT,) or mean.dtype != np.float64:
        raise RuntimeError(f"GeÃ§ersiz scaler mean: {arm} {mean.shape} {mean.dtype}")
    if scale.shape != (EXPECTED_FEATURE_COUNT,) or scale.dtype != np.float64:
        raise RuntimeError(f"GeÃ§ersiz scaler scale: {arm} {scale.shape} {scale.dtype}")
    if weights.shape != (3,) or weights.dtype != np.float32:
        raise RuntimeError(f"GeÃ§ersiz sÄ±nÄ±f aÄŸÄ±rlÄ±ÄŸÄ±: {arm} {weights.shape} {weights.dtype}")
    ...
    if not isinstance(split_audit, dict):
        raise RuntimeError(f"Ã–nbellekte Ã¶lÃ§eklenmiÅŸ split tam-tarama denetimi yok: {arm}")
    for split in SPLITS:
        ...
        if (
            ...
            raise RuntimeError(f"Ã–lÃ§eklenmiÅŸ split denetimi geÃ§ersiz: {arm}/{split}: {item}")
    overlap_audit = manifest.get("scaled_model_input_overlap_audit")
    if arm == "grouped_float32":
        if not isinstance(overlap_audit, dict):
            raise RuntimeError("Grouped Ã¶nbellekte Ã¶lÃ§eklenmiÅŸ model-girdisi denetimi yok.")
        if (
            ...
            raise RuntimeError(f"Grouped Ã¶lÃ§eklenmiÅŸ model-girdisi denetimi geÃ§ersiz: {overlap_audit}")
        audit_database = Path(str(overlap_audit.get("audit_database", "")))
        if not audit_database.exists():
            raise RuntimeError(f"Grouped Ã¶lÃ§eklenmiÅŸ girdi denetim DB bulunamadÄ±: {audit_database}")
        if sha256(audit_database) != overlap_audit.get("audit_database_sha256"):
            raise RuntimeError("Grouped Ã¶lÃ§eklenmiÅŸ girdi denetim DB SHA-256 uyuÅŸmuyor.")
...
def materialize_arm(
    ...
    if manifest_file.exists():
        ...
        if manifest.get("status") == "completed":
            ...
            print(f"[{arm}] TamamlanmÄ±ÅŸ VS2 Ã¶nbelleÄŸi kullanÄ±lÄ±yor.", flush=True)
            return manifest
    ...
    for file_number, (file_id, csv_path, raw_label, expected_rows) in enumerate(files, start=1):
        ...
        for frame in pd.read_csv(csv_path, chunksize=chunk_size):
            ...
            if values.shape[1] != EXPECTED_FEATURE_COUNT or not np.isfinite(values).all():
                raise RuntimeError(f"GeÃ§ersiz CSV Ã¶zellikleri: {csv_path}")
            end = base + len(values)
            for split in SPLITS:
            ...
            if now - last_file_progress >= 60.0:
                print(
                    f"[{arm}] Dosya {file_number}/{len(files)} iÅŸleniyor: {csv_path.name} "
                    f"({base:,}/{expected_rows:,} satÄ±r)",
                    flush=True,
                )
                last_file_progress = now
        if base != expected_rows:
            raise RuntimeError(
                f"CSV satÄ±r sayÄ±sÄ± uyuÅŸmuyor ({csv_path.name}): {base:,} != {expected_rows:,}"
            )
        print(f"[{arm}] Dosya {file_number}/{len(files)} hazÄ±rlandÄ±: {csv_path.name}", flush=True)
    ...
    for split in SPLITS:
        ...
        if observed_counts != expected_class_counts[split]:
            raise RuntimeError(
                f"{arm}/{split}: materyalize sÄ±nÄ±f sayÄ±larÄ± uyuÅŸmuyor: "
                f"{observed_counts} != {expected_class_counts[split]}"
            )
    ...
    if mean.shape != (EXPECTED_FEATURE_COUNT,) or standard_deviation.shape != (
        ...
        raise RuntimeError(f"{arm}: StandardScaler parametre boyutu geÃ§ersiz.")
    if not np.isfinite(mean).all() or not np.isfinite(standard_deviation).all():
        raise RuntimeError(f"{arm}: StandardScaler sonlu olmayan parametre Ã¼retti.")
    if np.any(standard_deviation <= 0):
        raise RuntimeError(f"{arm}: StandardScaler pozitif olmayan Ã¶lÃ§ek Ã¼retti.")
    ...
    if not scaler_audit_passed:
        raise RuntimeError(
            f"{arm}: StandardScaler tam tarama denetimi baÅŸarÄ±sÄ±z; "
    ...
    for split in SPLITS:
        ...
        if not split_finite or processed != counts[split]:
            raise RuntimeError(
                f"{arm}/{split}: Ã¶lÃ§eklenmiÅŸ model-girdisi sonluluk/satÄ±r denetimi baÅŸarÄ±sÄ±z."
            )
    ...
    if arm == "grouped_float32":
        print(
            "[grouped_float32] StandardScaler sonrasÄ± gerÃ§ek float32 model-girdisi "
            "Ã¶rtÃ¼ÅŸmesi denetleniyor...",
            flush=True,
        )
...
def protocol_settings(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if tuple(int(seed) for seed in protocol.get("training_seeds", ())) != SEEDS:
        raise RuntimeError(f"B0 protokol seed kÃ¼mesi uyuÅŸmuyor: {protocol.get('seeds')!r}")
    if str(protocol.get("task")) != "family_3":
        raise RuntimeError(f"B0 gÃ¶rev adÄ± uyuÅŸmuyor: {protocol.get('task')!r}")
    ...
    for key, value in expected.items():
        if str(common.get(key)) != value:
            raise RuntimeError(f"B0 protokolÃ¼ uyuÅŸmuyor: {key}={common.get(key)!r}")
    settings = {
        "optimizer": "AdamW",
    ...
    for key, expected_value in locked.items():
        if settings[key] != expected_value:
            raise RuntimeError(
                f"Kilitli B0 ayarÄ± deÄŸiÅŸmiÅŸ: {key}={settings[key]!r}, beklenen={expected_value!r}"
            )
...
def train_one(
    ...
    if result_file.exists():
        ...
        if mismatches:
            raise RuntimeError(f"TamamlanmÄ±ÅŸ VS2 sonucu uyumsuz: {result_file}: {mismatches}")
        print(f"[{arm}/seed{seed}] TamamlanmÄ±ÅŸ VS2 sonucu kullanÄ±lÄ±yor.", flush=True)
        return result
    if run_directory.exists() and any(run_directory.iterdir()):
        if not restart_incomplete:
            raise RuntimeError(
                f"YarÄ±m kalmÄ±ÅŸ run bulundu: {run_directory}. "
                "Ä°nceledikten sonra yalnÄ±z bu VS2 run iÃ§in --restart-incomplete kullanÄ±n."
            )
    ...
    if parameter_count != EXPECTED_PARAMETER_COUNT:
        raise RuntimeError(
            f"B0 tinyml_mlp parametre sayÄ±sÄ± deÄŸiÅŸmiÅŸ: {parameter_count:,} != {EXPECTED_PARAMETER_COUNT:,}"
        )
...
def main() -> None:
    ...
    if args.chunk_size <= 0:
        raise ValueError("--chunk-size pozitif olmalÄ±dÄ±r.")
    if tuple(args.seeds) != SEEDS:
        raise ValueError(f"Seed kÃ¼mesi kilitli olmalÄ±dÄ±r: {SEEDS}")
    if tuple(args.arms) != ARMS:
        print("UYARI: YalnÄ±z seÃ§ili kollar Ã§alÄ±ÅŸtÄ±rÄ±lÄ±yor; nihai eÅŸleÅŸtirilmiÅŸ rapor oluÅŸmayabilir.")
    ...
    print("Stage-37 bilimsel kilitleri doÄŸrulandÄ±; kaynak CSV Ã¶zetleri sÄ±nanÄ±yor...", flush=True)
    database_validation["source_integrity"] = validate_source_files(database, csv_root)
    print("Kaynak CSV bÃ¼tÃ¼nlÃ¼ÄŸÃ¼ doÄŸrulandÄ±; veritabanÄ± Ã¶zeti alÄ±nÄ±yor...", flush=True)
    ...
    if args.prepare_only:
        print("VS2 Ã¶nbellek hazÄ±rlama tamamlandÄ±; --prepare-only nedeniyle eÄŸitim baÅŸlatÄ±lmadÄ±.")
        return
    ...
    for arm in args.arms:
        for seed in args.seeds:
            ...
            print(f"[{arm}/seed{seed}] B0-eÅŸlenmiÅŸ eÄŸitim baÅŸlÄ±yor...", flush=True)
            result = train_one(
                arm_directory=cache / arm,
    ...
    if set(args.arms) == set(ARMS) and set(args.seeds) == set(SEEDS):
        ...
        print("=" * 78)
        print("38. aÅŸama tamamlandÄ±: B0-eÅŸlenmiÅŸ float32 sÄ±zÄ±ntÄ± ablasyonu")
        print(f"Naif Macro-F1 ortalamasÄ± : {macro['naive_row'].mean():.9f}")
        print(f"Gruplu Macro-F1 ortalamasÄ±: {macro['grouped_float32'].mean():.9f}")
        print(f"Naif - gruplu farkÄ±       : {differences.mean():.9f}")
        print("Eski 150 ana koÅŸu ve VS1 sonuÃ§larÄ± deÄŸiÅŸtirilmedi.")
        print("=" * 78)
