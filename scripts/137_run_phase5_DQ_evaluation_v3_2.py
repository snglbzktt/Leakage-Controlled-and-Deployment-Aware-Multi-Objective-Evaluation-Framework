...
def build_run_summary_from_files(
    ...
    return {
        ...
        "DQ_to_float_state_size_ratio": float(
            metrics["model_complexity"][
...
if failed_entry_checks:
    raise RuntimeError(
        "DQ evaluation entry gate failed: "
        + ", ".join(
            failed_entry_checks
        )
    )
...
if failed_shape_checks:
    raise RuntimeError(
        "DQ evaluation cache checks failed: "
        + ", ".join(
            failed_shape_checks
        )
    )
...
print("=" * 92)
print("PHASE 5 DYNAMIC QUANTIZATION EVALUATION")
print("=" * 92)
print(
    "Runs                            : 10"
)
...
print(
    "Source                          : Matching locked B0 checkpoint"
)
print(
    "Quantized dtype                 : torch.qint8"
)
print(
    "Quantization engine             : "
    f"{active_engine}"
...
print(
    "Test evaluation policy          : Exactly once per run"
)
print(
    "Free disk                       : "
...
for run_index, (
    ...
    preflight_row = preflight_lookup[
        (
            architecture,
            seed,
        )
    ...
    if run_directory.exists():
        ...
        if status_path.exists():
            ...
            if (
                ...
                raise RuntimeError(
                    "Incomplete DQ run has "
                    "already entered test "
    ...
    if (
        ...
        raise RuntimeError(
            f"{run_id}: float topology mismatch."
        )
    ...
    if (
        ...
        raise RuntimeError(
            ...
            "layers were dynamically quantized."
        )
    ...
    atomic_torch_save(
        {
            "artifact_type": (
                "phase5_dynamic_quantized_model"
            ),
            "protocol_version": (
                PROTOCOL_VERSION
            ),
    ...
    metrics = {
        ...
        "model_complexity": {
            ...
            "serialized_DQ_checkpoint_bytes": (
                int(
                    DQ_checkpoint_path
                    .stat()
                    .st_size
    ...
    print(
        ...
        "size_ratio="
        f"{DQ_state_bytes / float_state_bytes:.4f} | "
        "fragility="
...
if len(
    ...
    raise RuntimeError(
        "Expected ten completed DQ runs."
    )
...
for architecture in ARCHITECTURES:
    ...
    group_row = {
        ...
        "mean_validation_inference_seconds": (
            statistics[
                "validation_inference_seconds"
            ]["mean"]
        ),
...
print()
print("=" * 92)
print("PHASE 5 DYNAMIC QUANTIZATION SUMMARY")
print("=" * 92)
print(
    "Runs completed                  : 10"
)
print(
    "Source                          : Locked B0 checkpoints"
)
print(
    "Quantized dtype                 : torch.qint8"
)
...
for row in sorted(
    ...
    print(
        ...
        "state ratio="
        f"{float(row['mean_DQ_to_float_state_size_ratio']):.4f} | "
        "fragility="
...
print(
    "PHASE 5 DYNAMIC QUANTIZATION EVALUATION COMPLETED"
)
