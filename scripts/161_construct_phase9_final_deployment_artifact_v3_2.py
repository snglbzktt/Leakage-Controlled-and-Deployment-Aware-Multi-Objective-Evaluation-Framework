...
def discover_class_labels(
    ...
    if not candidates:
        raise RuntimeError(
            "No locked three-class label order "
            "was found in the selected metrics "
            "or checkpoint."
        )
    ...
    if len(unique_orders) != 1:
        ...
        raise RuntimeError(
            "Conflicting locked class-label "
            f"orders were found: {details}"
        )

    labels = list(
        next(iter(unique_orders))
    )
    ...
    if (
        ...
        raise RuntimeError(
            "Class labels are not unique."
        )

    sources = [
        path
        for _, path, candidate_labels
        in best_candidates
...
print("=" * 92)
print("PHASE 9 FINAL DEPLOYMENT ARTIFACT CONSTRUCTION")
print("=" * 92)
print(
