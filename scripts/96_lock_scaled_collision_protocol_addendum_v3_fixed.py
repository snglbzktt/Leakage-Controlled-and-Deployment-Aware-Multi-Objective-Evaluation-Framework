...
planned_moved_fingerprint_count = int(
    repair_plan.get(
        "proposed_moved_fingerprint_count",
        repair_plan.get(
            "minimum_moved_fingerprint_count",
            -1,
        ),
    )
)

planned_moved_raw_row_count = int(
    repair_plan.get(
        "proposed_moved_raw_row_count",
        repair_plan.get(
            "minimum_moved_raw_row_count_given_minimum_fingerprints",
            -1,
        ),
    )
)
...
checks = {
    ...
    "move_count_matches": (
        len(move_rows)
        == EXPECTED_MOVES
        == planned_moved_fingerprint_count
    ),
    "moved_raw_rows_match": (
        move_raw_row_sum
        == EXPECTED_MOVED_RAW_ROWS
        == planned_moved_raw_row_count
    ),
