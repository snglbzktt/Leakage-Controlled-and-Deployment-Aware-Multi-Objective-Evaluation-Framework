...
def load_model_module(
    ...
    module_name = (
        "phase5_tinyml_b0_verification_models"
    )
...
for path in (
    ...
    if path.exists():
        raise FileExistsError(
            "TinyML B0 verification artifact "
...
print("=" * 92)
print("PHASE 5 TINYML MLP B0 INDEPENDENT VERIFICATION")
print("=" * 92)
print(
    "Seeds                       : "
    f"{SEEDS}"
...
aggregate_checks.update(
    {
        ...
        "printed_mean_raw_weighted_matches": (
            close_enough(
                recomputed_aggregate[
                    "test_raw_weighted_macro_f1"
                ]["mean"],
...
if failed_aggregate_checks:
    raise RuntimeError(
        "TinyML B0 aggregate verification "
        "failed: "
        + ", ".join(
            failed_aggregate_checks
        )
...
verification = {
    ...
    "phase": 5,
    "artifact_name": (
        "tinyml_mlp_B0_five_seed_verification"
    ),
    "protocol_version": (
        PROTOCOL_VERSION
    ),
...
lock = {
    ...
    "ready_for_compact_dnn_B0_training": True,
    "all_checks_passed": True,
}
...
post_checks = {
    ...
    "ready_for_compact_dnn": (
        read_json(
            OUTPUT_LOCK
        ).get(
            "ready_for_compact_dnn_B0_training"
        )
        is True
    ),
}
...
if failed_post_checks:
    raise RuntimeError(
        "TinyML B0 post-lock checks failed: "
        + ", ".join(
            failed_post_checks
        )
    )
...
print()
print("=" * 92)
print("PHASE 5 TINYML MLP B0 VERIFICATION AND LOCK SUMMARY")
print("=" * 92)
print(
    "Verified runs                   : "
    f"{len(verified_runs)}"
...
print(
    "TinyML MLP B0 status            : LOCKED"
)
print(
    "Ready for Compact-DNN B0        : True"
)
print(
    "Verification report             : "
...
print(
    "PHASE 5 TINYML MLP B0 FIVE-SEED RUNS LOCKED"
)
