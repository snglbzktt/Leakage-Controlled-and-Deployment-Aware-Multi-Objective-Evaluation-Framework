...
RUNNER_SCRIPT = (
    ROOT
    / "scripts"
    / "108_run_random_forest_b0_five_seeds_v3_2.py"
)
...
if OUTPUT_REPORT.exists():
    raise FileExistsError(
        "Random Forest verification report already exists; "
...
if failed_global_checks:
    raise RuntimeError(
        "Random Forest global verification failed: "
...
print("=" * 92)
print("RANDOM FOREST B0 INDEPENDENT VERIFICATION")
print("=" * 92)
...
for run_number, row in enumerate(
    ...
    with np.load(
        ...
        saved_node_counts = np.array(
            parameters["tree_node_counts"],
            copy=True,
        )
        saved_depths = np.array(
            parameters["tree_depths"],
            copy=True,
        )
        saved_leaf_counts = np.array(
            parameters["tree_leaf_counts"],
            copy=True,
        )
    ...
    observed_node_counts = np.asarray(
        [
    ...
    observed_leaf_counts = np.asarray(
        [
    ...
    parameter_checks = {
        ...
        "tree_count_is_100": (
            len(model.estimators_) == 100
            and saved_node_counts.shape
            == (100,)
            and saved_depths.shape == (100,)
            and saved_leaf_counts.shape == (100,)
        ),
        ...
        "tree_leaf_counts_match": (
            np.array_equal(
                observed_leaf_counts,
                saved_leaf_counts,
            )
        ),
        "n_estimators_matches": (
            model.n_estimators
            == LOCKED_PARAMETERS[
                "n_estimators"
            ]
        ...
        "bootstrap_matches": (
            model.bootstrap
            is LOCKED_PARAMETERS["bootstrap"]
        ),
        "max_samples_matches": (
            close_enough(
                model.max_samples,
                LOCKED_PARAMETERS["max_samples"],
                tolerance=0.0,
            )
        ),
        ...
        "n_jobs_matches": (
            model.n_jobs
            == LOCKED_PARAMETERS["n_jobs"]
        ),
        "oob_score_matches": (
            model.oob_score
            is LOCKED_PARAMETERS["oob_score"]
        ),
    ...
    if failed_run_checks:
        raise RuntimeError(
            f"Random Forest verification failed "
...
print()
print("=" * 92)
print("RANDOM FOREST B0 VERIFICATION SUMMARY")
print("=" * 92)
print(
    "Runs verified                  : "
    f"{len(run_reports)}"
...
if failed_final_checks:
    ...
    raise RuntimeError(
        "Random Forest aggregate verification failed."
    )

print(
    "RANDOM FOREST B0 VERIFICATION COMPLETED"
)
