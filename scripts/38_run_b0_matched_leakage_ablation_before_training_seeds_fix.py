...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="N-BaIoT cihaz klasörlerini içeren kök dizin.",
    )
    ...
    parser.add_argument(
        "--restart-incomplete",
        action="store_true",
        help="Yalnız tamamlanmamış VS2 run klasörlerini silip yeniden başlatır.",
    )
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(document, dict):
        raise TypeError(f"JSON kökü sözlük değil: {path}")
    return document
...
def database_metadata(database: Path) -> dict[str, str]:
    ...
    for key, value in expected.items():
        if metadata.get(key) != value:
            raise RuntimeError(f"Split veritabanı metadata uyuşmazlığı: {key}={metadata.get(key)!r}")
    return metadata
...
def source_database_path(database: Path) -> Path:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Kaynak parmak izi veritabanı bulunamadı: {path}")
    return path.resolve()
...
def discover_files(source_database: Path, csv_root: Path) -> list[tuple[int, Path, str, int]]:
    with sqlite3.connect(source_database) as connection:
        ...
        if path_column is None or not required.issubset(columns):
            raise RuntimeError(f"Desteklenmeyen source.files şeması: {columns}")
...
def split_counts(database: Path, arm: str) -> dict[str, int]:
    ...
    if set(by_code) != {0, 1, 2}:
        raise RuntimeError(f"{arm} split kodları geçersiz: {by_code}")
    result = {name: by_code[code] for name, code in SPLIT_CODES.items()}
    if sum(result.values()) != EXPECTED_RAW_ROW_COUNT:
        raise RuntimeError(f"{arm} toplam kayıt sayısı geçersiz: {sum(result.values()):,}")
    return result
...
def split_class_counts(database: Path, arm: str) -> dict[str, list[int]]:
    ...
    for split, values in result.items():
        if any(value <= 0 for value in values):
            raise RuntimeError(f"{arm}/{split}: eksik family_3 sınıfı: {values}")
    return result
...
def validate_database(database: Path) -> dict[str, Any]:
    ...
    with sqlite3.connect(database) as connection:
        ...
        if quick_check.lower() != "ok":
            raise RuntimeError(f"Split veritabanı quick_check başarısız: {quick_check}")
    ...
    if failed:
        raise RuntimeError(f"Stage-37 bilimsel kilitleri başarısız: {failed}")
    return {
        "metadata": metadata,
...
def validate_source_files(database: Path, csv_root: Path) -> dict[str, Any]:
    ...
    if observed_source_database_sha256 != expected_source_database_sha256:
        raise RuntimeError(
            "Kaynak denetim veritabanı SHA-256 değeri stage 37 sonrasında değişmiş."
        )
    discovered = {
    ...
    if len(integrity_rows) != expected_file_count or set(discovered) != {
        ...
        raise RuntimeError("Kaynak CSV envanteri stage 37 ile uyuşmuyor.")
    total_bytes = 0
    for position, row in enumerate(integrity_rows, start=1):
        ...
        if current_path.resolve() != Path(str(stored_path)).resolve():
            raise RuntimeError(f"Kaynak CSV yolu değişmiş (file_id={file_id}).")
        if current_label != str(stored_label) or current_rows != int(stored_rows):
            raise RuntimeError(f"Kaynak CSV etiketi/satır sayısı değişmiş (file_id={file_id}).")
        stat = current_path.stat()
        if stat.st_size != int(stored_size):
            raise RuntimeError(f"Kaynak CSV boyutu değişmiş: {current_path}")
        observed_sha256 = sha256(current_path)
        if observed_sha256 != str(stored_sha256):
            raise RuntimeError(f"Kaynak CSV SHA-256 uyuşmuyor: {current_path}")
        total_bytes += int(stat.st_size)
        print(
            f"[kaynak doğrulama] {position}/{len(integrity_rows)}: {current_path.name}",
            flush=True,
        )
    return {
...
def inverse_sqrt_mean1(class_counts: np.ndarray) -> np.ndarray:
    ...
    if counts.shape != (3,) or np.any(counts <= 0):
        raise RuntimeError(f"Geçersiz eğitim sınıf sayıları: {counts.tolist()}")
...
def float32_fingerprints(values: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if matrix.ndim != 2 or matrix.shape[1] != EXPECTED_FEATURE_COUNT:
        raise RuntimeError(f"Model girdisi boyutu geçersiz: {matrix.shape}")
    if not np.isfinite(matrix).all():
        raise RuntimeError("Model girdisinde sonlu olmayan değer var.")
...
def audit_scaled_grouped_overlap(
    ...
    try:
        ...
        for split_code, split in enumerate(SPLITS):
            ...
            for start in range(0, len(x), chunk_size):
                ...
                if now - last_progress >= 60.0:
                    print(
                        f"[grouped_float32 gerçek model-girdisi denetimi] {split}: "
        ...
        quick_check = str(connection.execute("PRAGMA quick_check").fetchone()[0])
        if quick_check.lower() != "ok":
            raise RuntimeError(f"Ölçeklenmiş girdi denetim DB quick_check başarısız: {quick_check}")
    finally:
        connection.close()
    if processed_rows != EXPECTED_RAW_ROW_COUNT:
        raise RuntimeError(
            f"Ölçeklenmiş girdi denetim satır sayısı geçersiz: "
            f"{processed_rows:,} != {EXPECTED_RAW_ROW_COUNT:,}"
        )
    if set(unique_by_split) != set(SPLITS):
        raise RuntimeError(f"Ölçeklenmiş girdi split envanteri eksik: {unique_by_split}")
    if overlap_count != 0:
        raise RuntimeError(
            "Grouped kolda StandardScaler ve float32 dönüşümü sonrasında splitler arası "
            f"{overlap_count:,} model-girdisi çakışması bulundu; eğitim güvenli biçimde durduruldu."
        )
    return {
...
def ensure_cache_disk_space(database: Path, cache: Path, arms: list[str]) -> None:
    ...
    if free < required:
        raise RuntimeError(
            "VS2 önbelleği için boş alan yetersiz: "
            f"gereken yaklaşık={required / 1024**3:.2f} GiB, "
...
def validate_completed_cache(
    ...
    if mean.shape != (EXPECTED_FEATURE_COUNT,) or mean.dtype != np.float64:
        raise RuntimeError(f"Geçersiz scaler mean: {arm} {mean.shape} {mean.dtype}")
    if scale.shape != (EXPECTED_FEATURE_COUNT,) or scale.dtype != np.float64:
        raise RuntimeError(f"Geçersiz scaler scale: {arm} {scale.shape} {scale.dtype}")
    if weights.shape != (3,) or weights.dtype != np.float32:
        raise RuntimeError(f"Geçersiz sınıf ağırlığı: {arm} {weights.shape} {weights.dtype}")
    ...
    if not isinstance(split_audit, dict):
        raise RuntimeError(f"Önbellekte ölçeklenmiş split tam-tarama denetimi yok: {arm}")
    for split in SPLITS:
        ...
        if (
            ...
            raise RuntimeError(f"Ölçeklenmiş split denetimi geçersiz: {arm}/{split}: {item}")
    overlap_audit = manifest.get("scaled_model_input_overlap_audit")
    if arm == "grouped_float32":
        if not isinstance(overlap_audit, dict):
            raise RuntimeError("Grouped önbellekte ölçeklenmiş model-girdisi denetimi yok.")
        if (
            ...
            raise RuntimeError(f"Grouped ölçeklenmiş model-girdisi denetimi geçersiz: {overlap_audit}")
        audit_database = Path(str(overlap_audit.get("audit_database", "")))
        if not audit_database.exists():
            raise RuntimeError(f"Grouped ölçeklenmiş girdi denetim DB bulunamadı: {audit_database}")
        if sha256(audit_database) != overlap_audit.get("audit_database_sha256"):
            raise RuntimeError("Grouped ölçeklenmiş girdi denetim DB SHA-256 uyuşmuyor.")
...
def materialize_arm(
    ...
    if manifest_file.exists():
        ...
        if manifest.get("status") == "completed":
            ...
            print(f"[{arm}] Tamamlanmış VS2 önbelleği kullanılıyor.", flush=True)
            return manifest
    ...
    for file_number, (file_id, csv_path, raw_label, expected_rows) in enumerate(files, start=1):
        ...
        for frame in pd.read_csv(csv_path, chunksize=chunk_size):
            ...
            if now - last_file_progress >= 60.0:
                print(
                    f"[{arm}] Dosya {file_number}/{len(files)} işleniyor: {csv_path.name} "
        ...
        if base != expected_rows:
            raise RuntimeError(
                f"CSV satır sayısı uyuşmuyor ({csv_path.name}): {base:,} != {expected_rows:,}"
            )
        print(f"[{arm}] Dosya {file_number}/{len(files)} hazırlandı: {csv_path.name}", flush=True)
    ...
    for split in SPLITS:
        ...
        if observed_counts != expected_class_counts[split]:
            raise RuntimeError(
                f"{arm}/{split}: materyalize sınıf sayıları uyuşmuyor: "
                f"{observed_counts} != {expected_class_counts[split]}"
            )
    ...
    if mean.shape != (EXPECTED_FEATURE_COUNT,) or standard_deviation.shape != (
        ...
        raise RuntimeError(f"{arm}: StandardScaler parametre boyutu geçersiz.")
    if not np.isfinite(mean).all() or not np.isfinite(standard_deviation).all():
        raise RuntimeError(f"{arm}: StandardScaler sonlu olmayan parametre üretti.")
    if np.any(standard_deviation <= 0):
        raise RuntimeError(f"{arm}: StandardScaler pozitif olmayan ölçek üretti.")
    ...
    if not scaler_audit_passed:
        raise RuntimeError(
            f"{arm}: StandardScaler tam tarama denetimi başarısız; "
    ...
    for split in SPLITS:
        ...
        if not split_finite or processed != counts[split]:
            raise RuntimeError(
                f"{arm}/{split}: ölçeklenmiş model-girdisi sonluluk/satır denetimi başarısız."
            )
    ...
    if arm == "grouped_float32":
        print(
            "[grouped_float32] StandardScaler sonrası gerçek float32 model-girdisi "
            "örtüşmesi denetleniyor...",
            flush=True,
        )
...
def protocol_settings(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if tuple(int(seed) for seed in protocol.get("seeds", ())) != SEEDS:
        raise RuntimeError(f"B0 protokol seed kümesi uyuşmuyor: {protocol.get('seeds')!r}")
    if str(protocol.get("task")) != "family_3":
        raise RuntimeError(f"B0 görev adı uyuşmuyor: {protocol.get('task')!r}")
    ...
    for key, value in expected.items():
        if str(common.get(key)) != value:
            raise RuntimeError(f"B0 protokolü uyuşmuyor: {key}={common.get(key)!r}")
    settings = {
        "optimizer": "AdamW",
    ...
    for key, expected_value in locked.items():
        if settings[key] != expected_value:
            raise RuntimeError(
                f"Kilitli B0 ayarı değişmiş: {key}={settings[key]!r}, beklenen={expected_value!r}"
            )
...
def train_one(
    ...
    if result_file.exists():
        ...
        if mismatches:
            raise RuntimeError(f"Tamamlanmış VS2 sonucu uyumsuz: {result_file}: {mismatches}")
        print(f"[{arm}/seed{seed}] Tamamlanmış VS2 sonucu kullanılıyor.", flush=True)
        return result
    if run_directory.exists() and any(run_directory.iterdir()):
        if not restart_incomplete:
            raise RuntimeError(
                f"Yarım kalmış run bulundu: {run_directory}. "
                "İnceledikten sonra yalnız bu VS2 run için --restart-incomplete kullanın."
            )
    ...
    if parameter_count != EXPECTED_PARAMETER_COUNT:
        raise RuntimeError(
            f"B0 tinyml_mlp parametre sayısı değişmiş: {parameter_count:,} != {EXPECTED_PARAMETER_COUNT:,}"
        )
...
def main() -> None:
    ...
    if tuple(args.seeds) != SEEDS:
        raise ValueError(f"Seed kümesi kilitli olmalıdır: {SEEDS}")
    if tuple(args.arms) != ARMS:
        print("UYARI: Yalnız seçili kollar çalıştırılıyor; nihai eşleştirilmiş rapor oluşmayabilir.")
    ...
    print("Stage-37 bilimsel kilitleri doğrulandı; kaynak CSV özetleri sınanıyor...", flush=True)
    database_validation["source_integrity"] = validate_source_files(database, csv_root)
    print("Kaynak CSV bütünlüğü doğrulandı; veritabanı özeti alınıyor...", flush=True)
    ...
    if args.prepare_only:
        print("VS2 önbellek hazırlama tamamlandı; --prepare-only nedeniyle eğitim başlatılmadı.")
        return
    ...
    for arm in args.arms:
        for seed in args.seeds:
            ...
            print(f"[{arm}/seed{seed}] B0-eşlenmiş eğitim başlıyor...", flush=True)
            result = train_one(
                arm_directory=cache / arm,
    ...
    if set(args.arms) == set(ARMS) and set(args.seeds) == set(SEEDS):
        ...
        print("=" * 78)
        print("38. aşama tamamlandı: B0-eşlenmiş float32 sızıntı ablasyonu")
        print(f"Naif Macro-F1 ortalaması : {macro['naive_row'].mean():.9f}")
        print(f"Gruplu Macro-F1 ortalaması: {macro['grouped_float32'].mean():.9f}")
        print(f"Naif - gruplu farkı       : {differences.mean():.9f}")
        print("Eski 150 ana koşu ve VS1 sonuçları değiştirilmedi.")
        print("=" * 78)
